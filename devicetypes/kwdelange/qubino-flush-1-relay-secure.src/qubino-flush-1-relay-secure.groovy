/**
 *
 *  Qubino Flush 1 Relay Secure
 *   
 *	Modification of device handler of Eric Maycock (erocm123)
 *	Date: 2017-04-19
...
metadata {
	...
	definition (name: "Qubino Flush 1 Relay Secure", namespace: "kwdelange", author: "Koen de Lange") {
		capability "Actuator"
		capability "Switch"
		capability "Polling"
		capability "Refresh"
        ...
        fingerprint mfr: "0159", prod: "0002", model: "0052", cc: "5E,5A,73,98", sec: "86,72,27,25,32,71,85,8E,59,70"
	}
...
def installed() {
    ...
    // kwdelange: Check zwaveInfo to see if device is using security:
    if (getZwaveInfo()?.zw?.contains("s")) {
        logging("Device is securly paired. Using secure commands.", 1)
        state.sec = true
    }
	createChildDevices()
}
...
// kwdelange:zwaveEvent added for S0 security

def zwaveEvent(physicalgraph.zwave.commands.securityv1.SecurityMessageEncapsulation cmd) {
    logging("zwaveEvent(): Security Encapsulated Command received: ${cmd}", 2)
    state.sec = true
    def encapsulatedCommand = cmd.encapsulatedCommand([0x98: 1, 0x20: 1, 0x70: 1])
    if (encapsulatedCommand) {
         return zwaveEvent(encapsulatedCommand)
    } else {
        logging("zwaveEvent(): Unable to extract security encapsulated command from: ${cmd}", 1)
    }
}
...
def updated()
{
    ...
    // kwdelange: Check zwaveInfo to see if device is using security:
    if (getZwaveInfo()?.zw?.contains("s")) {
        logging("Device is securly paired. Using secure commands.", 2)
        state.sec = true
    }
    if (!childDevices) {
		createChildDevices()
...
{
    logging("update_needed_settings() started", 2)
    def cmds = []
    def currentProperties = state.currentProperties ?: [:]
...
def zwaveEvent(physicalgraph.zwave.commands.configurationv1.ConfigurationReport cmd) {
     logging("Starting update_current_properties", 1)
